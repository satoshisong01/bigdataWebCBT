...
import { getHistory, getWrongIds, getActiveWrongIds, getBookmarks, getAllExamAnswers, clearAllWrongStats, getStreak, StreakData, ExamRecord } from '@/lib/storage';
...
export default function HomePage() {
  ...
  const [streak, setStreak] = useState<StreakData>({ days: [], current: 0, best: 0 });
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-slate-950 text-gray-900 dark:text-slate-100">
      <div className="bg-white dark:bg-slate-900 border-b border-gray-200 dark:border-slate-800 relative">
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 py-10 sm:py-16 text-center">
          ...
          <p className="text-gray-500 dark:text-slate-400 mt-2 sm:mt-3 text-base sm:text-lg">기출문제 풀이 및 해설</p>
          <div className="flex justify-center gap-4 sm:gap-8 mt-4 sm:mt-6 text-xs sm:text-sm text-gray-400 dark:text-slate-500">
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-6 sm:py-10 space-y-8 sm:space-y-12">
        ...
        {(wrongPoolCount > 0 || bookmarkCount > 0) && (
          <section>
            <h2 className="text-lg sm:text-xl font-bold text-gray-800 dark:text-slate-100 mb-3 sm:mb-4">학습 풀</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
              {wrongPoolCount > 0 && (
                ...
                >
                  <div className="flex items-center justify-between">
                    <div>
                      <div className="text-base sm:text-lg font-bold text-gray-800 dark:text-slate-100 group-hover:text-red-600 transition">
                        오답노트
                      </div>
              ...
              {bookmarkCount > 0 && (
                ...
                >
                  <div className="flex items-center justify-between">
                    <div>
                      <div className="text-base sm:text-lg font-bold text-gray-800 dark:text-slate-100 group-hover:text-yellow-600 transition">
                        ★ 북마크
                      </div>
                      <div className="text-xs sm:text-sm text-gray-400 dark:text-slate-500 mt-1">
                        문제 풀이 중 ☆ 클릭하여 추가
        ...
        {/* 회차별 풀기 */}
        <section>
          <h2 className="text-lg sm:text-xl font-bold text-gray-800 dark:text-slate-100 mb-3 sm:mb-4">회차별 풀기</h2>
              ...
              return (
                ...
                >
                  ...
                  <Link href={`/exam?mode=session&id=${session.id}`} className="block group">
                    <div className="text-base sm:text-lg font-bold text-gray-800 dark:text-slate-100 group-hover:text-blue-600 transition">
                      {session.name}
                    </div>
                    <div className="text-xs sm:text-sm text-gray-400 dark:text-slate-500 mt-1">
        ...
        {/* 과목별 풀기 */}
        <section>
          <h2 className="text-lg sm:text-xl font-bold text-gray-800 dark:text-slate-100 mb-3 sm:mb-4">과목별 풀기</h2>
          <div className="space-y-3 sm:space-y-4">
            {SUBJECTS.map(subject => (
              <div
                key={subject.id}
                className="bg-white dark:bg-slate-900 rounded-xl border border-gray-200 dark:border-slate-800 p-4 sm:p-5"
              >
                <div className="flex items-center gap-2 sm:gap-3 mb-3 sm:mb-4">
                  <span className="text-xs sm:text-sm text-blue-600 font-bold">{subject.id}과목</span>
                  <span className="text-sm sm:text-base font-bold text-gray-800 dark:text-slate-100">
                    {subject.name}
                  </span>
                  <span className="text-xs text-gray-400 dark:text-slate-500 hidden sm:inline">회차당 20문항</span>
                </div>
                        ...
                        <Link
                          ...
                          className={`inline-flex items-center gap-1 sm:gap-1.5 px-2.5 sm:px-3 py-1.5 sm:py-2 rounded-lg text-xs sm:text-sm font-medium transition w-full sm:w-auto justify-center sm:justify-start ${
                            record
                              ...
                              : 'bg-gray-50 dark:bg-slate-950 text-gray-700 dark:text-slate-300 border border-gray-200 dark:border-slate-800 hover:bg-blue-50 hover:text-blue-700 hover:border-blue-200 active:bg-blue-100'
                          }`}
                        >
        ...
        {/* 안내 */}
        <section className="bg-white dark:bg-slate-900 p-4 sm:p-6 rounded-xl border border-gray-200 dark:border-slate-800">
          <h3 className="font-bold text-gray-800 dark:text-slate-100 mb-2 sm:mb-3 text-sm sm:text-base">시험 안내</h3>
          <div className="text-xs sm:text-sm text-gray-500 dark:text-slate-400 space-y-1 sm:space-y-1.5">
          ...
          <div className="text-center pb-4 flex flex-col sm:flex-row gap-2 sm:gap-4 justify-center">
            {completedCount > 0 && (
              <button
                ...
                className="text-xs sm:text-sm text-gray-400 dark:text-slate-500 hover:text-red-500 active:text-red-600 transition"
              >
                풀이 기록 초기화
              </button>
            )}
